...
class RAGASEvaluator:
    ...
    def __init__(
        ...
        if not RAGAS_AVAILABLE or not LANGCHAIN_AVAILABLE:
            raise ImportError(
                "RAGAS and LangChain OpenAI are required. Install with: pip install ragas langchain-openai"
            )
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            logger.info(
                f"\u2705 RAGAS evaluator initialized with Faithfulness and ContextRecall metrics"
            )
    ...
    def evaluate_batch(
        ...
        if len(queries) != len(responses) or len(queries) != len(contexts_list):
            raise ValueError(
                "Queries, responses, and contexts must have the same length"
            )
...
async def evaluate_rag_response(
    query: str, response: str, contexts: List[str], ground_truth: Optional[str] = None
) -> Dict[str, float]:
    """

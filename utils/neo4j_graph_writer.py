...
class Neo4jGraphWriter:
    ...
    def get_graph_stats(self) -> Dict:
        ...
        try:
            with self.driver.session() as session:
                ...
                stats = {}
                labels = ["Topic", "Task", "Person", "Role", "Department", "Organization", "Date", "Summary", "EmailIndex"]
    ...
    def write_tasks_from_table(self, tasks_data: List[Dict], clear_existing: bool = False) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ Successfully wrote {len(tasks_data)} tasks to Neo4j from table structure")
            return True
        except Exception as e:
    ...
    def _write_single_task_from_table(self, session, task_row: Dict):
        ...
        """, topic_name=topic_name, task_name=task_name)
        ...
        if sender:
            ...
            """, task_name=task_name, sender=sender)
        ...
        if assigned_to:
            ...
            """, task_name=task_name, assigned_to=assigned_to)
        ...
        if due_date:
            session.run("""
                ...
                MATCH (task:Task {name: $task_name})
                MERGE (task)-[:DUE_ON]->(d)
            """, date=str(due_date), task_name=task_name)
        ...
        if received_date:
            ...
            """, date=str(received_date), task_name=task_name)
        ...
        if message_id:
            session.run("""
                ...
                MERGE (task)-[:LINKED_TO]->(e)
                        """, message_id=message_id, task_name=task_name)

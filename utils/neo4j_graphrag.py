...
class Neo4jGraphRAG:
        ...
        with self.driver.session() as session:
            ...
            if texts:
                try:
                    ...
                    logger.info(
                        f"✅ Computed OpenAI embeddings for {len(self.node_embeddings)} nodes"
                    )
    ...
    def expand_from_nodes(
        self, start_node_ids: List[str], max_nodes: int = 25, direct_only: bool = False
    ) -> set:
    ...
    def query(self, query: str, direct_only: bool = False, max_nodes: int = 25) -> Dict:
        ...
        if topic_matches:
            ...
            explanation = (
                f"Found {len(all_nodes)} nodes from {len(topic_ids)} related topic(s)"
            )
            if direct_only:
            ...
            return {
                ...
                "relevant_nodes": [
                    (name, score)
                    for name, score in self._convert_topic_matches_to_names(
                        topic_matches
                    )
                    if score >= 0.5
                ],
        ...
        return {"query": query, "error": error_msg, "nodes": [], "method": "no_match"}

    def search_topics_by_name(
        self, query: str, semantic_threshold: float = 0.5
            ...
            with self.driver.session() as session:
                ...
                for record in result:
                    ...
                    if node_id in self.node_embeddings:
                        ...
                        if (
                            query_lower in topic_name_lower
                            or any(
                                word in topic_name_lower for word in query_lower.split()
                            )
                            or similarity >= semantic_threshold
                        ):
    ...
    def _convert_topic_matches_to_names(
        self, topic_matches: List[Tuple[str, float]]
    ) -> List[Tuple[str, float]]:
        ...
        with self.driver.session() as session:
            ...
            return [
                (id_to_name.get(node_id, "Unknown"), score)
                for node_id, score in topic_matches
        ...
        try:
            ...
            with self.driver.session() as session:
                ...
                for record in rel_result:
                    net.add_edge(
                        record["from_node"], record["to_node"], label=record["rel_type"]
                    )
    ...
    def query_flexible(self, query: str, max_nodes: int = 25) -> Dict:
        ...
        all_node_ids = self.expand_from_nodes(
            matched_nodes, max_nodes=max_nodes, direct_only=False
        )
        all_nodes = self._get_node_names_from_ids(list(all_node_ids))
        matched_names = [
            node_id_to_name[nid] for nid in matched_nodes if nid in node_id_to_name
        ]
        matched_labels = [
            node_id_to_label[nid] for nid in matched_nodes if nid in node_id_to_label
        ]
    ...
    def query_with_semantic_reasoning(self, query: str) -> Dict:
        ...
        if result.get("all_nodes"):
            try:
                ...
                contexts = self._extract_contexts_from_nodes(
                    result.get("all_nodes", [])
                )
                ...
                try:
                    ...
                    if loop.is_running():
                        ...
                        ragas_scores = {}
                        logger.info(
                            "Async context detected - RAGAS evaluation skipped in real-time"
                        )
                    else:
                        ...
                        ragas_scores = asyncio.run(
                            evaluator.evaluate_single(
                                query, response, contexts, ground_truth=ground_truth
                            )
                        )
                except:
                ...
                if ragas_scores:
                    ...
                    result["ragas_summary"] = evaluator.format_evaluation_summary(
                        ragas_scores
                    )
                    ...
                    result["ragas_summary"] = (
                        "⚠️ RAGAS evaluation unavailable. No RAGAS scores could be computed."
                    )
                ...
                result["ragas_overall"] = result.get("confidence_score", 0.0)
                result["ragas_summary"] = (
                    f"📊 **Quick Assessment:** {result.get('confidence_score', 0.0):.3f}"
                )
    ...
    def _extract_contexts_from_nodes(self, node_names: List[str]) -> List[str]:
        ...
        if not contexts:
            contexts = [
                f"Graph contains information about: {', '.join(node_names[:5])}"
            ]
            ...
            for task_name in task_names:
                ...
                for record in detail_result:
                    ...
                    if record["email_index"]:
                        task_info.append(
                            f"   • **Email Index:** {record['email_index']}"
                        )
                    if record["owner_name"]:
                        task_info.append(
                            f"   • **Responsible To:** {record['owner_name']}"
                        )

                    collaborators = [c for c in record["collaborators"] if c]
                    if collaborators:
                        task_info.append(
                            f"   • **Collaborated By:** {', '.join(collaborators)}"
                        )

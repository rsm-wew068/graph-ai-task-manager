...
with col2:
    selected_year = st.selectbox(
        "Year", options=list(range(1980, 2007)), index=default_date.year - 1980
    )

if st.button("📅 Go to Selected Month", type="primary"):
    st.session_state.calendar_start_date = datetime.date(
...
if database_tasks:
    ...
    if dates:
        ...
        max_date = max(dates)
        st.info(
            f"📊 Showing {len(database_tasks)} tasks from database (Date range: {min_date} to {max_date})"
        )
    else:
...
if tasks is None:
    st.warning(
        "No tasks found. Please upload and process emails from the " "main page first."
    )
    st.info(
    ...
    if events:
        ...
        if (
            st.session_state.show_task_modal
            and st.session_state.selected_task
            and st.session_state.selected_task in task_details
        ):
            ...
            with st.expander("📝 Task Details", expanded=True):
                ...
                if task_info.get("priority_level"):
                    priority_color = {"High": "🔴", "Medium": "🟡", "Low": "🟢"}.get(
                        task_info["priority_level"], "⚪"
                    )
                    st.markdown(
                        f"• **Priority:** {priority_color} {task_info['priority_level']}"
                    )
        ...
        st.info("Tasks will appear here once you process emails with " "valid dates.")

...
st.set_page_config(page_title="Automated Task Manager", page_icon="📧", layout="wide")
...
print(
    f"Utils __init__.py exists: {os.path.exists(os.path.join(current_dir, 'utils', '__init__.py'))}"
)
...
try:
    ...
    st.error(
        "Please ensure all utils files are properly uploaded to your Hugging Face Space"
    )
    st.stop()
    ...
    try:
        ...
        if db.connect():
            ...
            st.session_state["uploaded_file_name"] = (
                f"Filtered from PostgreSQL ({len(df_emails)} emails)"
            )
    ...
    if st.button("📧 Load 100 Recent Emails"):
        ...
        try:
            ...
            if db.connect():
                with db.connection.cursor() as cursor:
                    cursor.execute(
                        "SELECT * FROM parsed_email ORDER BY date_received DESC LIMIT 100"
                    )
                    emails = [dict(row) for row in cursor.fetchall()]
                ...
                st.session_state["parsing_complete"] = True
                st.session_state["uploaded_file_name"] = (
                    "Recent 100 emails from PostgreSQL"
                )
    ...
    if st.button("📧 Load 500 Random Emails"):
        ...
        try:
            ...
            if db.connect():
                with db.connection.cursor() as cursor:
                    cursor.execute(
                        "SELECT * FROM parsed_email ORDER BY RANDOM() LIMIT 500"
                    )
                    emails = [dict(row) for row in cursor.fetchall()]
                ...
                st.session_state["parsing_complete"] = True
                st.session_state["uploaded_file_name"] = (
                    "Random 500 emails from PostgreSQL"
                )
                    ...
                    rows.append(
                        {
                            ...
                            "Owner Name": (
                                ...
                                else str(owner) if owner else "Unknown"
                            ),
                            "Owner Role": (
                                owner.get("role", "") if isinstance(owner, dict) else ""
                            ),
                            "Owner Department": (
                                owner.get("department", "")
                                if isinstance(owner, dict)
                                else ""
                            ),
                            "Owner Organization": (
                        ...
                        rows.append(
                            {
                                ...
                                "Collaborator Name": (
                                    collab.get("name", "Unknown")
                                    if isinstance(collab, dict)
                                    else str(collab) if collab else "Unknown"
                                ),
                                "Collaborator Role": (
                                    collab.get("role", "")
                                    if isinstance(collab, dict)
                                ...
                                "Collaborator Department": (
                                    collab.get("department", "")
                                    if isinstance(collab, dict)
                                    else ""
                                ),
                                "Collaborator Organization": (
                                    collab.get("organization", "")
                                    if isinstance(collab, dict)
                                    else ""
                                ),
                                "Email Index": task_obj.get("email_index", ""),
...
if st.session_state.parsing_complete and st.session_state.parsed_emails is not None:
    ...
    with st.expander("📊 Email Data Details", expanded=False):
        ...
        with col2:
            ...
            if all(
                col in df_emails.columns
                for col in ["from_email", "subject", "date_received"]
            ):
                sample_df = df_emails[["from_email", "subject", "date_received"]].head(
                    3
                )
                ...
                with st.status("Running LLM extraction...") as status:
                    for i, (_, email_row) in enumerate(emails_to_process.iterrows()):
                        ...
                        try:
                            ...
                            print(
                                f"📧 DEBUG: Subject: {email_row.get('Subject', 'No Subject')}"
                            )
                            ...
                            print(
                                f"📊 DEBUG: Status: {result.get('status', 'unknown')}"
                            )
                            print(f"📊 DEBUG: Valid: {result.get('valid', 'not set')}")
                            print(
                                f"📊 DEBUG: Has validated_json: {'validated_json' in result}"
                            )

                            # Show debug info in Streamlit if enabled
                            if st.session_state.get("debug_mode", False):
                                st.info(
                                    f"Email {i+1}: Status={result.get('status')}, Valid={result.get('valid')}"
                                )
                            ...
                            email_content = email_row.get(
                                "Body", email_row.get("Content", "")
                            )
                            ...
                            outputs.append(
                                {
                                    ...
                                    "correctable_json": json.dumps(
                                        template_json, indent=2
                                    ),
                                    "valid": False,
...
if st.session_state.processing_complete and st.session_state.extracted_tasks:
    ...
    with st.expander("📊 Detailed Result Analysis", expanded=False):
        ...
        for i, res in enumerate(outputs):
            ...
            if "validated_json" in res:
                st.write(
                    f"- Validated JSON preview: `{str(res['validated_json'])[:100000]}...`"
                )
            elif "extracted_json" in res:
                st.write(
                    f"- Extracted JSON preview: `{str(res['extracted_json'])[:100000]}...`"
                )
    ...
    for i, res in enumerate(outputs):
        ...
        if res.get("status") in ["paused", "awaiting_user_review"] or res.get(
    ...
    if valid_tasks:
        st.success(
            f"✅ Successfully extracted {len(valid_tasks)} valid tasks from {len(outputs)} processed emails."
        )
    if paused_results:
        st.warning(
            f"⚠️ {len(paused_results)} emails require manual review. See the Human Validation Required section below."
        )
    ...
    if paused_results:
        ...
        for idx, result in paused_results:
            ...
            with col2:
                ...
                correctable_template = result.get(
                    "correctable_json", result.get("extracted_json", "{}")
                )
    ...
    if all_valid_tasks:
            ...
            if st.button("💾 Store to Databases", type="primary"):
                try:
                    ...
                    if valid_tasks_to_store:
                        ...
                        # Store to Neo4j
                        neo4j_writer = Neo4jGraphWriter()
                        neo4j_success = neo4j_writer.write_tasks_from_table(
                            valid_tasks_to_store, clear_existing=False
                        )

                        st.success(
                            f"✅ Successfully stored {len(task_ids)} tasks to databases!"
                        )
        ...
        with col2:
            if st.session_state.get("data_stored", False):
                ...
                st.info(
                    "💡 Click 'Store to Databases' to save extracted tasks to PostgreSQL and Neo4j"
                )
    else:
